...
This work reduces computational cost and improves interpretability in structure-based drug discovery by filtering low-quality poses before expensive refinement and surfacing failure modes a generative model has not learned. Because it filters existing poses rather than generating new molecules, it does not lower the barrier to designing toxic compounds---the field's primary dual-use concern. A more realistic risk is uneven performance: our filters are highly case-specific (Section 3.4), so precision may not transfer outside the 36 systems studied here, and over-reliance without periodic validation could deprioritize rarer disease targets.
